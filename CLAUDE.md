...
- **`src/web/`** — Streamlit UI. `app.py` builds shared sidebar state (ticker, model, Monte Carlo/Heston/jump params) and passes it into four tab modules under `src/web/tabs/` (`tab_option_analysis`, `tab_scanner`, `tab_backtester`, `tab_portfolio_risk`), each exposing a `render(...)` function. Shared UI/data helpers (cached fetchers, `model_key`, formatters, table styling, expiration selection) live in `src/web/common.py` — reuse these rather than re-adding per-tab copies. Model-fit diagnostics live inside the scanner tab (there is no separate validation tab).
